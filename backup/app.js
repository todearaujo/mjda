...
function destacarBloco( numerobloco, texto ) {
  ...
  for ( let bloco of blocosgrafico ) {
    ...
    bloco.setAttribute('onclick', 'expandirGeracao(' + numerobloco + ');criaFatos(' + numerobloco + ')')
...
const criaFatos = async () => await (await fetch(gjson)).json().then(infos,n => {

  const geracoes = infos.ids
  const fs = infos.fatos

  for (geracao of geracoes) {
    if (geracao.id === n)
      console.log('ID' + n + ", início em " + geracao.inicio + " e fim em " + geracao.fim)
      for (fato of fs){
        while (geracao.fim > fato.quando > geracao.inicio) {
          let posicao = 100 - ( ( geracao.fim - fato.quando ) * 100 / ( geracao.fim - geracao.inicio ) )
          let f = document.createElement('div')
          f.setAttribute('data-posicao', posicao)
          f.setAttribute('data-inicio', geracao.inicio)
          f.setAttribute('data-fim', geracao.fim)
