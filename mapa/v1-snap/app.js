...
const CONFIG = {
  ...
  bulgeFactor: 0.7,        // intensidade do estufamento (zoom-out) na viagem entre estados
  bulgeMax: 1.7,           // teto do estufamento
  travelDur: 720           // duração (ms) da viagem da câmera entre estados
};
...
// --- Câmera por estado (viewBox animado, disparada por IntersectionObserver) ---
let cameras = [];
let stepEls = [];
let activeIdx = -1;
let observer = null;
...
// proporcional à distância: perto = pan suave; longe = sobrevoo que abre o mapa
// antes de pousar no próximo estado. Mesma matemática da v1 contínua (enquadra
// todo estado certo, inclusive os grandes do oeste). Em t=1 retorna b exato.
...
// Viaja a câmera até o enquadramento-alvo escrevendo o atributo viewBox quadro a
// quadro (rAF). Sem transform: o viewBox É a câmera, como na v1 que já está no ar,
// então o enquadramento de cada estado é o comprovado. animate:false pousa direto.
const animateViewBoxTo = (target, animate) => {
  ...
  const from = currentViewBox || target;
  let start = null;
  const frame = (ts) => {
    if (start === null) start = ts;
    const raw = Math.min((ts - start) / CONFIG.travelDur, 1);
    const eased = raw * raw * (3 - 2 * raw); // smoothstep
    const cam = travelViewBox(from, target, eased);
    mapSvg.setAttribute("viewBox", viewBoxString(cam));
    currentViewBox = cam;
    if (raw < 1) {
...
// Ativa um painel: viaja a câmera (viewBox), atualiza o card, a barra e a borda.
const setActive = (index, animate = true) => {
  if (index < 0 || index >= cameras.length || index === activeIdx) return;
  activeIdx = index;
  animateViewBoxTo(cameras[index], animate);
...
const init = async () => {
  ...
  requestAnimationFrame(() => {
    computeCameras();
    setActive(0, false); // Brasil (intro): pousa direto, sem animação no 1º paint
    setupObserver();
  });
  ...
  window.addEventListener("resize", () => {
    ...
    resizeTimer = setTimeout(() => {
      computeCameras();
      if (activeIdx >= 0) animateViewBoxTo(cameras[activeIdx], false);
